...
describe('gendiff', () => {
  ...
  test('compare nested JSON structures', () => {
    ...
    expect(result).toMatch(/^\s* {2}setting1: Value 1$/m)
    ...
    expect(result).toMatch(/^\s* {4}key5: value5$/m)

    expect(result).toMatch(/^\s* {2}setting6: {$/m)
    expect(result).toMatch(/^\s* {4}doge: {$/m)
    expect(result).toMatch(/^\s*- wow: $/m)
    expect(result).toMatch(/^\s*\+ wow: so much$/m)
    expect(result).toMatch(/^\s* {4}key: value$/m)
    expect(result).toMatch(/^\s*\+ ops: vops$/m)

    expect(result).toMatch(/^\s*- baz: bas$/m)
    expect(result).toMatch(/^\s*\+ baz: bars$/m)
    expect(result).toMatch(/^\s* {2}foo: bar$/m)
